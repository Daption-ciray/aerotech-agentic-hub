...
export function Dashboard() {
  ...
  if (loading) {
    return (
      <aside className="w-80 flex-shrink-0 border-l border-slate-800 bg-slate-925 flex flex-col items-center justify-center">
  ...
  return (
    <aside className="w-80 flex-shrink-0 border-l border-slate-800 bg-slate-925 flex flex-col overflow-hidden">
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-900/50 p-4">
          ...
          <div className="flex gap-1">
            <input
              ...
              className="flex-1 rounded border border-slate-700 bg-slate-800 px-2 py-1.5 text-xs text-zinc-100 placeholder:text-zinc-500"
            />
            <button
          ...
          {sprintResult && (
            <div className="mt-2 rounded border border-slate-700 bg-slate-800/50 p-2 text-xs">
              {sprintResult.error ? (
                <p className="text-amber-400">{String(sprintResult.error)}</p>
              ) : null}
              ...
              {sprintResult.operation === "update_status" && sprintResult.item ? (
                <p className="text-emerald-400">Durum güncellendi: {String((sprintResult.item as Record<string, unknown>).title ?? "")}</p>
              ) : null}
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-900/50 p-4">
          ...
          <div className="mt-3">
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className="text-zinc-300 font-mono">{progressPct}%</span>
            </div>
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-900/50 p-4">
          ...
          <div className="space-y-2">
            {recentItems.map((item) => (
              <div
                key={item.id}
                className="flex items-start justify-between gap-2 py-2 border-b border-slate-800 last:border-0"
              >
