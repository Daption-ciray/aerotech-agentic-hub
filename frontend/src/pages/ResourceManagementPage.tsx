  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
      <div className="flex-shrink-0 px-6 py-4 border-b border-slate-800">
        <h2 className="text-lg font-semibold text-zinc-100">Kaynak & Ekipman Yönetimi</h2>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        danger ? "text-thy-red hover:bg-thy-red/20" : "text-zinc-400 hover:text-zinc-100"
      )}
    >
  ...
  return (
    <div className="space-y-3">
      {personnel.map((p) => (
        ...
        >
          <div>
            <p className="text-sm font-medium text-zinc-100">{String(p.name ?? "")}</p>
            ...
            <div className="flex items-center gap-1">
              <span
                className={cn(
                  ...
                  stock === 0 ? "bg-thy-red/20 text-thy-red" : stock <= 2 ? "bg-amber-500/20 text-amber-400" : "bg-emerald-500/20 text-emerald-400"
                )}
              >
                Stok: {stock}
              </span>
    ...
    <CrudModal title={editing ? "Personel Düzenle" : "Yeni Personel"} open onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          ...
          <input value={id} onChange={(e) => setId(e.target.value)} disabled={!!editing} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" placeholder="P006" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Ad Soyad</label>
          <input value={name} onChange={(e) => setName(e.target.value)} required className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Rol</label>
          <input value={role} onChange={(e) => setRole(e.target.value)} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Ratings (virgülle)</label>
          <input value={ratings} onChange={(e) => setRatings(e.target.value)} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" placeholder="B1, B2" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Uzmanlık (virgülle)</label>
          <input value={specializations} onChange={(e) => setSpecializations(e.target.value)} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" placeholder="flight_controls" />
        </div>
  ...
  return (
    <CrudModal title={editing ? "Tool Düzenle" : "Yeni Tool"} open onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label className="block text-xs text-zinc-500 mb-1">ID</label>
          <input value={id} onChange={(e) => setId(e.target.value)} disabled={!!editing} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" placeholder="T006" />
        </div>
        <div>
        ...
        <div>
          ...
          <input value={category} onChange={(e) => setCategory(e.target.value)} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Konum</label>
          <input value={location} onChange={(e) => setLocation(e.target.value)} required className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
        </div>
        <div>
  ...
  return (
    <CrudModal title={editing ? "Parça Düzenle" : "Yeni Parça"} open onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label className="block text-xs text-zinc-500 mb-1">ID</label>
          <input value={id} onChange={(e) => setId(e.target.value)} disabled={!!editing} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Parça No</label>
          <input value={part_no} onChange={(e) => setPartNo(e.target.value)} required className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Ad</label>
          <input value={name} onChange={(e) => setName(e.target.value)} required className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
        ...
        <div className="flex gap-2">
          <div>
            ...
            <input value={ata_chapter} onChange={(e) => setAtaChapter(e.target.value)} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
          </div>
          <div>
            <label className="block text-xs text-zinc-500 mb-1">Stok</label>
            <input type="number" value={stock_level} onChange={(e) => setStockLevel(parseInt(e.target.value) || 0)} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
          </div>
        </div>
        <div>
          <label className="block text-xs text-zinc-500 mb-1">Konum</label>
        ...
        <div>
          ...
          <input type="number" value={lead_time_days} onChange={(e) => setLeadTimeDays(parseInt(e.target.value) || 0)} className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-zinc-100" />
        </div>
        <div className="flex gap-2 pt-2">
          <button type="submit" disabled={saving} className="flex-1 rounded bg-thy-red px-4 py-2 text-sm font-medium text-white hover:bg-thy-red-hover disabled:opacity-50">
            {saving ? "..." : "Kaydet"}
